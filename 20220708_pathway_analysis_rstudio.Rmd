...
```
BiocManager::install(c("pathview", "edgeR", "gage"))
```
...
```{r}
##you can use your own count data here: 

rnaseq_data <- read.table("rnaseq_data.txt", header=TRUE)
...
```{r}
## need to change this for your data
...
```{r}
##you can use your organism code here 
...
range(norm_counts)
head(norm_counts)
write.csv(x = norm_counts, file = "norm_counts.csv")
```
...
```q.val```	FDR q-value adjustment of the global p-value using the Benjamini & Hochberg procedure implemented in multitest package. This is the default q-value being used.
...
### 9. Getting DE gene set and fold change
...
### 6. Import pathway file into html document

```{r img-with-knitr, echo=FALSE, fig.align='center', out.width='100%', fig.cap='Caption for this figure 1'}
knitr::include_graphics("/cloud/project/ath03008.Wt_hy.png")

```


### 7. See how the output look like.
